...
if (isset($input['callback_query'])) {
    ...
    if (in_array($data, ['lang_ru', 'lang_en'])) {
        $lang = ($data === 'lang_ru') ? 'ru' : 'en';
        ...
        $msg = ($lang === 'ru') ? "Вы выбрали русский язык. Пожалуйста, введите ваше имя и фамилию." : "You chose English. Please enter your full name.";
...
switch ($step) {
    case 'waiting_name':
        ...
        file_put_contents($user_data_file, json_encode($users, JSON_PRETTY_PRINT));
        $msg = ($user_lang === 'ru') ? "Спасибо, {$text}! Введите ваш номер телефона Telegram." : "Thanks, {$text}! Please enter your Telegram phone number.";
        sendMessage($chat_id, $msg);
        break;
    ...
    case 'waiting_phone':
        ...
        file_put_contents($user_data_file, json_encode($users, JSON_PRETTY_PRINT));
        $msg = ($user_lang === 'ru') ? "Номер получен! Введите ваш Email." : "Phone received! Please enter your email.";
        sendMessage($chat_id, $msg);
        break;
    ...
    case 'waiting_email':
        ...
        file_put_contents($user_data_file, json_encode($users, JSON_PRETTY_PRINT));
        $msg = ($user_lang === 'ru') ? "Спасибо! Теперь введите вашу страну." : "Thank you! Now please enter your country.";
        sendMessage($chat_id, $msg);
        break;
    ...
    case 'waiting_payment_check_confirm':
        ...
        if (($user_lang === 'ru' && ($answer === 'да' || $answer === 'yes')) || ($user_lang === 'en' && $answer === 'yes')) {
            ...
            file_put_contents($user_data_file, json_encode($users, JSON_PRETTY_PRINT));
            $msg = ($user_lang === 'ru') ? "Операция отменена." : "Operation cancelled.";
            sendMessage($chat_id, $msg);
        }
        break;
